module Paperclip
  class Avconv < Processor
    ...
    def make
      Avconv.log("Making...") if @whiny
      src = @file
      Avconv.log("Building Destination File: '#{@basename}' + '#{@format}'") if @whiny
      dst = Tempfile.new([@basename, @format ? ".#{@format}" : ''])
      Avconv.log("Destination File Built") if @whiny
      dst.binmode
      
      parameters = []

      Avconv.log("Adding Geometry") if @whiny
      # Add geometry
      if @geometry
        Avconv.log("Extracting Target Dimensions") if @whiny
        ...
        unless @meta[:size].nil?
          Avconv.log("Target Size is Available") if @whiny
          current_width, current_height = @meta[:size].split('x')
          # Current width and height
          if @keep_aspect
            Avconv.log("Keeping Aspect Ratio") if @whiny
            if @enlarge_only
              Avconv.log("Enlarge Only") if @whiny
              if current_width.to_i < target_width.to_i
                ...
                Avconv.log("Convert Options: #{@convert_options[:output][:s]}") if @whiny
              else
                Avconv.log("Source is Larger than Destination, Doing Nothing") if @whiny
                #return nil
              end
            elsif @shrink_only
              Avconv.log("Shrink Only") if @whiny
              if current_width.to_i > target_width.to_i
                # Keep aspect ratio
                width = target_width.to_i
                height = (width.to_f / (@meta[:aspect].to_f)).to_i
                ...
                Avconv.log("Source is Smaller than Destination, Doing Nothing") if @whiny
                #return nil
              end
            elsif @pad_only
              Avconv.log("Pad Only") if @whiny
              # Keep aspect ratio
              width = target_width.to_i
              height = (width.to_f / (@meta[:aspect].to_f)).to_i
            ...
            Avconv.log("Not Keeping Aspect Ratio") if @whiny
      ...
      if @auto_rotate && !@meta[:rotate].nil?
        Avconv.log("Adding rotation #{@meta[:rotate]}") if @whiny
        case @meta[:rotate]
        when 90
      ...
      Avconv.log("Adding Format") if @whiny
      # Add format
      case @format
      ...
      Avconv.log("Adding Source") if @whiny
      # Add source
      ...
      Avconv.log("Building Parameters") if @whiny
      parameters = parameters.flatten.compact.join(" ").strip.squeeze(" ")

      Avconv.log(parameters)
      begin
        success = Paperclip.run("avconv", parameters, :source => "#{File.expand_path(src.path)}", :dest => File.expand_path(dst.path))
    ...
    def identify
      ...
      Paperclip.log("[avconv] Command Success") if @whiny
      meta
    end


    def self.log message
      Paperclip.log "[avconv] #{message}"
    end
  end
